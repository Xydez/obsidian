...
Tre (fyra) steg:
- Initiation
- Elongation (with translocation)
- Termination

### Initiation
**Initiering**
Ribosomens subenheter sätts ihop runt mRNA:t.

 **Förklaring** enl. Ninja Nerd, boken, Wikipedia och Hussain Biology:
 1. 30S-subenheten binder genom dess 16S-rRNA till Shine-Dalgarnosekvensen (och ligger då på utsidan av mRNA:t) varvid IF1 avhåller aminoacyl-tRNA från A-site och IF3 avhåller 50S-subenheten från att binda för tidigt.
 ...
 3. IF1 och IF3 släpper pga. strukturella förändringar. Därefter rekryterar sistnämnda IF2 50S-subenheten, varvid dess GTP bryts ned (hydrolyseras) till GDP+Pi. Detta erhåller tillräckligt med energi för att binda 50S-subenheten, varvid även IF2 släpper.
...
Bakteriell proteinsyntes initieras av Initiator tRNA (N-formylmetionyl-transfer RNA tRNAi^fMet).
...
- IF2 är en medlem av G-proteinfamiljen som binder GTP; denna binder efter IF1/IF3 och tillåter dem två att släppa
	- Denna släpper sedan också när 50S och 30S-subenheterna sammanfogar? svar: ja, se ovan



### Elongering
**Elongation**
Elongering är processen varvid en ny aminosyra adderas till peptidkedjan. *Elongation factors* (EF) hjälper till vid denna process.

1. $\ce{\text{EF-Tu}-\text{GTP}}$ binder $\ce{\text{aminoacyl}-\text{tRNA}}$ och levererar den till A-site; när aminosyran binder förbrukas en fosfatgrupp och $\ce{\text{EF-Tu}-\text{GDP}+P_i}$ avges.
2. EF-T klyver GDP från EF-Tu, varvid en ny GTP binder och bildar en återladdad $\ce{\text{EF-Tu}-\text{GTP}}$.

Härefter måste de två aminosyrorna ligeras. I prokaryoter har 50S-subenheten ett *peptidyltransferascenter* som överför peptidkedjan till N-terminalen på aminosyran i A-site.

![[Prokaryotic elongation.png]]

%%
Följande beskriver processen för eukaryoter:

För eukaryoter kallas dessa eukaryotic elongation factors (EeF).

EeF-1 bär med sig en GTP och hjälper till att placera aminosyran i P-site varvid \$\ce{\text{GDP}+P_i}\$ avges

Enzymet peptidyltransferas hjälper sedan till att överföra peptiden från P-site till N-terminalen på A-site
%%

**Translocation**
Translokering innebär att peptidkedjan flyttas ett steg.

Elongation factor G (EF-G, även kallat *translokas*) använder GTP för att flytta peptidkedjan.

%%
Följande beskriver processen för eukaryoter:

När peptidkedjan flyttas ett steg. EeF-2 bär med sig en GTP och flyttar peptidkedjan ett steg varvid \$\ce{GDP + P_i}\$ avges.
%%
...
### Terminering
**Termination**
Terminering är processen varvid translationen medelst så kallade *release factors* (RF1-RF3) avslutas.

RF1 och RF2 känner igen olika stoppkodon (UAA och UAG respektive UAA och UGA).

RF3 är en GTPas som katalyserar borttagning av RF1/RF2 från ribosomen efter att det nybildade proteinet har lämnat.
